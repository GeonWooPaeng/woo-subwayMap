...
function checkValidNameLength(inputLineName) {
  if (inputLineName.length < MINIMUM_LINE_LENGTH) {
    return false;
  }
  return true;
}
...
function checkValidInput(inputLineName) {
  ...
  if (!checkValidNameLength(inputLineName)) {
    return alert(`노선 이름은 ${MINIMUM_LINE_LENGTH}글자 이상이어야 합니다.`);
  }
...
function getLineName() {
  const inputLineName = document
    .querySelector('#line-name-input')
    .ariaValueMax.trim();
